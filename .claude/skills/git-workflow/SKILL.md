...
4. **Land the bump on `develop` via PR.** Both `main` and `develop` are protected — no
   ...
   # ^ The CHANGELOG [X.Y.Z] section also becomes the Play "What's new" (generated by
   #   scripts/changelog-to-whatsnew.sh, capped at 500 chars) — keep it user-facing + concise.
